...
export function TabProgramacion({ atletaId }: { atletaId: string }) {
  ...
  // La semana actual (hoy cae dentro de su rango) arranca desplegada; el resto, colapsado.
  const hoy = hoyIso();
  const bloqueActualId = bloquesOrdenados.find((b) => b.fechaInicio <= hoy && hoy <= b.fechaFin)?.id;
        ...
        <div className="space-y-6">
          {bloquesOrdenados.map((bloque) => (
            <ProgramacionView
              key={bloque.id}
              bloque={bloque}
              defaultOpen={bloque.id === bloqueActualId}
              onEditarSesion={setSesionSeleccionada}
            />
          ))}
        </div>
      )}
